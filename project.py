...
async def start_lesson(callback_query: types.CallbackQuery):
    ...
    await bot.send_message(
        ...
        "Пожалуйста, отправьте текст или фото для обработки.",
    )

    # В конце урока выводим результат ученика
    await bot.send_message(
        callback_query.from_user.id, "Результат ученика: [текст ученика]"
    )
...
async def ai_assistant(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    await bot.send_message(
        callback_query.from_user.id,
        "Вы выбрали ИИ-ассистента. Спросите об интересующих вас темах по физике.",
    )
